<template>
  <Dialog v-model:visible="visible" modal header="Edit Role" :draggable="false" :style="{ width: '40rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
...
const saveRole = async () => {
  ...
  try {
    const response = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/roles/${roleId.value}`, {
      name: name.value,
...
const showDialog = (data) => {
  ...
  selectedPermissions.value = data.permissions.map(key => {
    const foundPermission = permissions.value.find(p => p.key == key);
    return foundPermission ? foundPermission.name : null;
  });
  visible.value = true;
}
